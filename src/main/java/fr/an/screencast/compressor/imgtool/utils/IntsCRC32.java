 ...
 * 
 *  internal optimisation: ... tested with / without allocating/copying data 
 *  results: better to copy data in buffer!!
 *   small size => direct in memory compute
 *   medium size => copy to buffer + compute
 *   big size => split by blocks and copy to (pooled) buffer + compute
 */
public final class IntsCRC32 {

    /* private to force all static */
    private IntsCRC32() {
    }
    
    private static final int BLOCK_INTS_COUNT = 1024;
    private static final int BLOCK_BYTES_SIZE = BLOCK_INTS_COUNT * 4;
    
    private static final int BENCHMARK_MAXINTS_DIRECT = 1024;
    private static final int BENCHMARK_MAXINTS_COPYALLOC = 1024; // do not use?? prefer lock+reuse buffer (less memory GC)
    
    private static ReentrantLock lock = new ReentrantLock(); 
    private static ByteBuffer pooledBuffer = null;
    ...
    private static ByteBuffer allocBuffer() {
        ...
        lock.unlock();
        if (res == null) {
            res = ByteBuffer.allocate(BLOCK_BYTES_SIZE);
        }
        return res;
    }
    
    private static void freeBuffer(ByteBuffer p) {
        lock.lock();
        pooledBuffer = p;
        lock.unlock();
    }
    ...
    private static final int[] CRC_TABLE_256 = {
    ...
    public static int crc32Update(int crc, byte[] data, int offset, int len) {
        final int maxI = offset + len;
        for(int i = offset; i < maxI; i++) {
            crc = (crc >>> 8) ^ CRC_TABLE_256[(crc ^ data[i]) & 0xff];
        }
        return crc;
    }

    public static int crc32Update(int crc, byte value) {
        return (crc >>> 8) ^ CRC_TABLE_256[(crc ^ value) & 0xff];
    }
    ...
    public static int crc32Done(int crc) {
        return crc ^ 0xffffffff;
    }
    
    public static int crc32Update(int crc, int[] data, int offset, int len) {
        final int maxI = offset + len;
        for(int i = offset; i < maxI; i++) {
            // cf DataOutpuStream
            int v = data[i];
    ...
    public static int crc32InMemory(byte[] data, int offset, int len) {
        return crc32Done(crc32Update(crc32Init(), data, offset, len));
    }

    public static int crc32InMemory(int[] data, int offset, int len) {
        return crc32Done(crc32Update(crc32Init(), data, offset, len));
    }
